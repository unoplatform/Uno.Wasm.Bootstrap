...
class Driver {
	...
	void Run (string[] args) {
		...
		var enable_debug_arg = enable_debug ? "--debug" : "";

		// Rules
		ninja.WriteLine ("rule aot");
		ninja.WriteLine ($"  command = MONO_PATH=$mono_path $cross {enable_debug_arg} {profiler_aot_args} --aot=llvmonly,asmonly,no-opt,static,direct-icalls,llvm-outfile=$outfile $src_file");
