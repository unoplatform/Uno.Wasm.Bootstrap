...
No `dotnet publish` shelling, no `RemoveDir` hacks, no fingerprint fixup needed.

## Known Limitations

1. **Build-time assets only**: The host integration uses the worker's BUILD output, not publish output. This means the worker doesn't go through AOT/relinking during the host's publish. For AOT workers, the worker project should be published separately first.

2. **`_framework/` files come from build output glob**: The worker's `_framework/` files aren't in the SDK's build manifest (they're generated by a separate pipeline). We glob them from the build output wwwroot and register via `DefineStaticWebAssets`.
