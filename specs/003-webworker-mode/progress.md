...
1. **Hybrid pipeline**: Non-framework assets (worker.js, package_*, etc.) flow through the static web assets manifest via `GetCurrentProjectBuildStaticWebAssetItems`. However, `_framework/` files are only generated during publish (via `WasmTriggerPublishApp`), so they require an `AfterTargets="Publish"` target that runs `dotnet publish` on the worker and copies `_framework/` into the host output. The fingerprint in `uno-config.js` is fixed up after this copy.
